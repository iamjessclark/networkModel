...
split.nodes.sir<- 
  function(n.subnodes, u0, adj = NULL) { # removing seir= arguemnt
    ...
    transitions.tab <-
      ...
      # Mosquitos ####
        ...
        em.trans <- 
          ...
          #paste0("Em", sn, " -> ",Nm, " > 0 ? extrinInc*Em", sn, ": 0 -> Im", sn)
        ...
        jm.trans.D <-
          ...
          #paste0("Jm", sn, " ->", Nm, " > 0 ? mu*Jm", sn, " : 0 -> @")
          
        sm.trans.D <-
          paste0("Sm", sn, " -> mu*Sm", sn, " -> @")
          #paste0("Sm", sn, " ->", Nm, " > 0 ? mu*Sm", sn, " : 0 -> @")
        
        em.trans.D <-
          paste0("Em", sn, " -> mu*Em", sn, " -> @")
          #paste0("Em", sn, " ->", Nm, " > 0 ? mu*Em", sn, " : 0 -> @")
        
        im.trans.D <-
          paste0("Im", sn, " -> mu*Im", sn, " -> @")
          #paste0("Im", sn, " ->", Nm, " > 0 ? mu*Im", sn, " : 0 -> @")
    ...
    Ev[substr(rownames(Ev), 1, 2) == "Ih", 6] <- 1 # just infected hosts
    Ev[substr(rownames(Ev), 1, 4) == "Im", 3] <- 1 # just infected adults
    Ev[substr(rownames(Ev), 1, 2) == "Sm"|
         substr(rownames(Ev), 1, 2) == "Em"|
         substr(rownames(Ev), 1, 2) == "Im", 4] <- 1 # all adult mosqs
    Ev[substr(rownames(Ev), 1, 2) == "Sm", 5] <- 1 # just susceptible adults
    Ev <- rbind(Ev, Ic = 0)
